...
```json
{
  ...
  "method": "GET",
  "path": "/api/v1/data/notes",
  "params": { "createdAtFrom": "1733097600000", "limit": "1000" },
  "pages": 10
}
```

`method` 省略時は `GET`。`pages` を 2 以上にすると `offset` を `limit × ページ番号` で
インクリメントしながら逐次取得し、`duration_ms` には全ページの累計時間が記録される。
例: `limit=1000, pages=10` で約 10,000 件取得のレイテンシを計測できる。
