...
def test_posts_get_limit_two_and_paginate(client: TestClient, post_samples: List[Post]) -> None:
    response = client.get("/api/v1/data/posts/?limit=2")
    assert response.status_code == 200
    res_json = response.json()
    assert res_json == {
        "data": [json.loads(post_samples[0].model_dump_json()), json.loads(post_samples[1].model_dump_json())],
        "meta": {"next": None, "prev": None},
    }
