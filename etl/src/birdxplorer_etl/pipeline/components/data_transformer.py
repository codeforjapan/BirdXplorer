...
class DataTransformerComponent(PipelineComponent):
    ...
    def _transform_notes(self, sqlite: Session, output_dir: str) -> Optional[str]:
        ...
        start_time = self.get_config_value(
            "target_start_unix_millisecond", TARGET_NOTE_ESTIMATE_TOPIC_START_UNIX_MILLISECOND
        )
    ...
    def _transform_media(self, postgresql: Session, output_dir: str) -> list[str]:
        ...
        with (
            ...
            # Write headers
            media_writer = csv.DictWriter(
                media_csv, fieldnames=["media_key", "type", "url", "width", "height", "post_id"]
            )
