...
def lambda_handler(event, context):
    ...
    try:
        ...
        for record in event["Records"]:
            try:
                ...
                if operation == "insert_note":
                    ...
                    postgresql.execute(
                        update(RowNoteRecord).where(RowNoteRecord.note_id == note_id).values(language=language)
                    )
                    postgresql.commit()
                    ...
                    existing_associations = (
                        postgresql.query(NoteTopicAssociation).filter(NoteTopicAssociation.note_id == note_id).all()
                    )
                    ...
                    if topic_ids:
                        ...
                        existing_topic_ids = (
                            postgresql.query(TopicRecord.topic_id).filter(TopicRecord.topic_id.in_(topic_ids)).all()
                        )
        ...
        return {"statusCode": 200, "body": json.dumps({"message": "DB write operations completed"})}
